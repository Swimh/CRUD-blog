<x-layout>
    <x-panel>
        <main class="max-w-6xl mx-auto mt-6 lg:mt-10 space-y-6 "> 
            <form method="post" action="{{route('update.category',['category'=>$category->id])}}" enctype="multipart/form-data" class="w-full max-w-lg  m-auto">
                @csrf
                @method('patch')
                ...
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full px-3">
                        ...
                        <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="name" type="text" name="name" placeholder="Example: Family" value="{{$category->name}}">
                    </div>
                </div>                
